...
function create_nft_item(game_user,item_desc_json)
    ...
    local dealer=contract_base_info.owner
    -- TODO create_nh_asset 目前还不支持创建的时候指定 dealer，合约可添加新的创建异质资产的接口，支持指定 dealer
    local asset_id=chainhelper:create_nh_asset2(game_user,dealer,world_view,item_desc_json,true)
...
function sell_nft_item(nft_asset_id,price_amount,price_symbol,memo,expiration_time)
    ...
    -- TODO 向 owner 授予转移 NFT 权力，目前还没有 approve_nft_from_caller 合约接口，需要开发
    chainhelper:approve_nft_from_caller(owner,nft_asset_id,true)
...
function buy_nft_item(nft_asset_id)
    ...
    -- TODO owner 将 NFT 委托转移给买方, 目前还没有 delegate_transfer_nft_from_caller，需要开发
    chainhelper:delegate_transfer_nft_from_caller(buyer,nft_asset_id,true)
