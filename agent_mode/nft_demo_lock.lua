...
参数:
    ...
    commissions_json: 扣除手续费配置参数json字符串，如{"1.2.5":0.05}表示给用户1.2.5提取5%的手续费用
--]] 
...
function sell_nft_item(nft_asset_id,price_amount,price_symbol,memo,expiration_time)
    ...
    -- 向合约转移 NFT token, 并进行资产锁定操作
    local owner=contract_base_info.owner
    chainhelper:transfer_nht_from_caller(owner,nft_asset_id,true)
    chainhelper:adjust_lock_nft_asset(nft_asset_id,true)
...
function buy_nft_item(nft_asset_id)
    ...
    -- 合约解锁 NFT 资产，并转移给买方
    chainhelper:adjust_lock_nft_asset(nft_asset_id,false)
    chainhelper:change_nht_active_by_owner(buyer,nft_asset_id,true)
    chainhelper:transfer_nft_ownership_from_owner(buyer,nft_asset_id,true)
