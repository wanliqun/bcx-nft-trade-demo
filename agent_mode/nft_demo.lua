--[[ 
    NFT交易合约中介模式 - 合约仅做连接买卖双方的渠道，不持有买卖双方资产，收取固定分配比例的手续费。
...
--]] 
function init(commissions_json)
    ...
    write_list={public_data={is_init=true,tbl_charge_settings=true}}
...
--]] 
function sell_nft(nft_asset_id,price_amount,price_symbol,memo,expiration_time)
    ...
    read_list={public_data={is_init=true,tbl_nft_orders=true}}
    chainhelper:read_chain()
    ...
    chainhelper:transfer_nht_from_caller(contract_base_info.owner,nft_asset_id,true) --[[转入NFT]]
    -- TODO 目前尚不支持锁定转出被转入的 NFT（功能尚待开发）
    ...
    -- TODO 卖单数据是否需要写入用户的 private data？
...
function buy_nft(nft_asset_id)
    ...
    -- TODO 目前尚不支持解锁被转入的 NFT（功能尚待开发）
    chainhelper:transfer_nht_from_owner(buyer,nft_asset_id,true) --[[转出NFT]]
